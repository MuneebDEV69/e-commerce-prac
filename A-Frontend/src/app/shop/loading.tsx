...
/**
 * Shown instantly while the /shop route loads (route-transition Suspense boundary).
 * A clear spinner + message up top, then a skeleton that mirrors the real layout
 * (sub-category row + 4-col grid) so there's no layout shift when data arrives.
 */
export default function ShopLoading() {
  return (
    <div className="pb-12">
      {/* Prominent loading banner (so the state is unmistakable when it appears) */}
      <div className="flex items-center justify-center gap-3 py-6 text-brand">
        <Loader2 size={20} className="animate-spin" strokeWidth={2} />
        <span className="text-sm tracking-[0.25em]">SHOP PAGE IS LOADING…</span>
      </div>

      <div className="animate-pulse">
        {/* sub-category circles */}
        <div className="flex items-center gap-6 px-4 md:px-10 pb-6">
